...
class EditContacts extends Component {
	...
	allCountacts = () => {
		...
		const mappedList = list.map((contact) => (
			<li key={contact.id} className={'list-group-item clearfix  displayEditlist'}>
				{' '}
				<img src={this.imgSrc(contact)} alt="icon" className="thumbnail ml-2" />
				<span>
					{' '}
					{contact.firstName} {contact.lastName}
				</span>
				<button onClick={(e) => this.props.editContact(e, contact.id, 'editContacts')} className="btn btn-info">
					Edit Contact
				</button>
			</li>
		));
